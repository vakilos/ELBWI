...
parser.add_argument("-i", dest="rootDir", help="Project directory - (should contain qiime2_dada2 directory)", required=True)
args=parser.parse_args()
### Global variables
rootDir = args.rootDir  #'/media/christos/ssd/work/Infants/publication/'
...
"""Remove cross-contaminant ASVs due to barcode bleeding
In the first (main) sequencing run, there is no other dataset with 16S amplicons, 
so it is safe to assume there is no crosscontamination from other samples. 
Using the crosstabulated table from the second sequencing run i am trying to exclude 
ASVs (otus in the table) that are represented with more reads in a sample that derives 
from another dataset. To finally determine the contaminants, those ASVs should be additionaly
either absent in the first sequencing run or in worst case detected in very low numbers 
and not systematically. I looked at phylum, order, family and genus level, 
due to the differences in the resolution of the taxonomic assignment. 
For instance, Cyanobacteria phulym was removed, 
as ASVs that derive from this phulym are detecting only in the second run in which there 
are other datasets with much higher abundance for these ASVs.
"""
...
"""
Find read cutoff for rarefying
We choose 200 reads per sample as the cutoff that maximizes the number of samples 
...
""" Clustering """
"""Silhouette analysis can be used to study the separation distance between clusters. 
...
number of clusters visually. This measure has a range of [-1, 1] A value of 0 indicates 
that the sample is on or very close to the decision boundary between two neighboring clusters
 and negative values indicate that those samples might have been assigned to the wrong 
 cluster. Ideally, all individual cluster Silhouette scores should be above the average 
 Silhouette score and have small fluctuations in the size of the Silhouette plot. 
 The thickness of the silhouette plot indicates the cluster size. Here, we plot the 
 silhouette score for each cluster, together with the average silhouette score, for 
 a total number of clusters ranging from 2 to 10. We find that 4 clusters can be best 
 justified/supported from our data.
"""
