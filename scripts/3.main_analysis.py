...
##pd.read_clipboard().to_csv('/media/christos/ssd/work/Infants/envFit_output.tsv', index=False,sep='\t') ## copies output to clipboard to create an output file
#envfit_process_for_R('/media/christos/ssd/work/Infants/envFit_output.tsv')
### I manually saved the output on a google spreadsheet

""" Exports this 'envFit_to_R.tsv' to be used with envfit_final_plot.R"""
...
print('\nFig. 5A')
#transitions = cluster_transition_prob_per_bodysite(master, clusterNameColumn= 'ClusterASV')
transitions = transition_prob_from_master(master) ### newer function for transition probs.

transitions.to_csv('Transitions.csv',index=False)

## export also file with Cluster Frequencies per Bodysite for comparison with steady state frequencies
cluster_freq_per_bodysite(master).to_csv('ClusterFreq_per_bodysite.csv',index=False)
...
""" For supplementrary figures run suppl_figures"""
sys.exit()
...
### Relative abundances of the differentially abundant ASVs for IVH
r_dif = relativeAbundance_from_dataframe(asv)
r_dif = r_dif[['ASV_164:Prevotella','ASV_61:Gardnerella','ASV_112:Pseudomonas','ASV_82:Staphylococcus']]
r_dif = r_dif.merge(master[['IVH']], on='SampleID', how='inner')
m_dif = r_dif.melt(id_vars=['IVH'])
m_dif.to_csv('/media/christos/ssd/work/Infants/tmp/DiffAbundantASVforIVH.tsv',sep='\t', index=False)


### Relative abundances of the differentially abundant ASVs for BPD
r_dif = relativeAbundance_from_dataframe(asv)
r_dif = r_dif[["ASV_25:Proteus","ASV_139:Proteus","ASV_199:Pseudomonas"]]
r_dif = r_dif.merge(master[['BPD']], on='SampleID', how='inner')
m_dif = r_dif.melt(id_vars=['BPD'])
m_dif.to_csv('/media/christos/ssd/work/Infants/tmp/DiffAbundantASVforBPD.tsv',sep='\t', index=False)
